...
inline void
    listStorageControllers(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                           const std::string& storagePath,
                           const std::string& storageId)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, storageId](const boost::system::error_code ec,
                               const std::vector<std::string>& ctrl_paths) {
        if (ec)
        {
            BMCWEB_LOG_ERROR << "listStorageControllers error";
            messages::internalError(asyncResp->res);
            return;
        }

        asyncResp->res.jsonValue["Members@odata.count"] = ctrl_paths.size();
        nlohmann::json& members = asyncResp->res.jsonValue["Members"];
        members = nlohmann::json::array();
        for (const auto& c : ctrl_paths)
        {
            auto name = sdbusplus::message::object_path(c).filename();
        ...
        "xyz.openbmc_project.ObjectMapper",
        "/xyz/openbmc_project/object_mapper",
        "xyz.openbmc_project.ObjectMapper", "GetSubTreePaths", storagePath, 0,
        std::array<const char*, 1>{
            "xyz.openbmc_project.Inventory.Item.StorageController"});
}
...
inline void requestRoutesStorage(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/Storage/<str>/")
        ...
        crow::connections::systemBus->async_method_call(
            ...
            // TODO(matt): populate health rollup from child controllers
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/Storage/<str>/Controllers")
        ...
        crow::connections::systemBus->async_method_call(
            ...
            asyncResp->res.jsonValue["Id"] = "Controllers";

            listStorageControllers(asyncResp, storage->first, storageId);
            },
            "xyz.openbmc_project.ObjectMapper",
            "/xyz/openbmc_project/object_mapper",
            "xyz.openbmc_project.ObjectMapper", "GetSubTree",
    ...
    const dbus::utility::MapperServiceMap& ifaces)
{
    ...
    crow::utility::appendUrlPieces(sendUrl,
        "Actions", "StorageController.SecuritySend");
    boost::urls::url receiveUrl(controllerUrl);
    crow::utility::appendUrlPieces(receiveUrl,
        "Actions", "StorageController.SecurityReceive");
    ...
    const std::string& path, const dbus::utility::MapperServiceMap& ifaces)
{
    ...
    asyncResp->res.jsonValue["Name"] =
        std::string("Storage Controller ") + controllerId;
    ...
    // TODO(matt): fill health
    // TODO(matt): add SerialNumber etc, eg populateStorageControllers()
}
...
inline void requestRoutesStorageController(App& app)
{
    BMCWEB_ROUTE(app,
        ...
        findStorageController(
            asyncResp, storageId, controllerId,
            [asyncResp, storageId,
             controllerId](const std::string& path,
                           const dbus::utility::MapperServiceMap& ifaces) {
            getStorageController(asyncResp, storageId, controllerId, path,
                                 ifaces);
            });
        });
}
    ...
    uint8_t proto, uint16_t protoSpecific, const std::string& dataBase64)
{
    ...
    auto service = matchServiceName(ifaces,
            "xyz.openbmc_project.Inventory.Item.StorageControllerSecurity");
    if (!service)
    {
        ...
        findStorageController(
            ...
            [asyncResp, proto, protoSpecific,
             transferLength](const std::string& path,
                              const dbus::utility::MapperServiceMap& ifaces) {
            securityReceiveAction(asyncResp, path, ifaces, proto,
